...
if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '2.7.0', '<' ) ) {
	include_once("legacy/2.6/cart.php");
	return;
}
if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '3.3.0', '<' ) ) {
	include_once("legacy/3.2/cart.php");
	return;
}
wc_print_notices();
					...
					<tr class="tm-epo-cart-row-product woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="product-subtotal" data-title="<?php esc_attr_e( 'Total', 'woocommerce' ); ?>">
							...
							if (TM_EPO()->tm_epo_cart_field_display=="advanced"){
								if (TM_EPO()->tm_epo_hide_options_in_cart=="normal"){
									if (isset($cart_item['tm_epo_product_after_adjustment']) && isset(TM_EPO()->tm_epo_dpd_enable) && TM_EPO()->tm_epo_dpd_enable=="no"){
										$price = $cart_item['tm_epo_product_after_adjustment'];
									}else{
										$price=apply_filters('wc_epo_discounted_price', $cart_item['tm_epo_product_original_price'], wc_get_product(tc_get_id($cart_item['data'])), $cart_item_key, true);
									}
								...
								echo apply_filters( 'wc_tm_epo_ac_subtotal_price', 
									apply_filters( 'woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal( $original_product, $cart_item['quantity'] ), $cart_item, $cart_item_key ), 
										$cart_item_key, 
										$cart_item, 
										$_product, 
										$product_id );
