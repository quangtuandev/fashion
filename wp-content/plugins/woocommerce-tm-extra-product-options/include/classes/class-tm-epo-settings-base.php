		...
		return array(
			...
			array(
				...
				'options'  => array(
					...
					'options'             => __( 'Show only Options total', 'woocommerce-tm-extra-product-options' ),
	...
	public function _get_setting_license( $setting, $label ) {
		...
		$_license_settings = (!defined( 'TM_DISABLE_LICENSE' )) ?
			array(
				...
				array(
					'title'    => __( 'Envato API Personal Token', 'woocommerce-tm-extra-product-options' ),
					'desc_tip' => '<span>' . __( 'Your Envato API Personal Token.', 'woocommerce-tm-extra-product-options' ) . '</span>',
					'desc'     => '<p>' . sprintf( __( 'You can generate an Envato API Personal Token by %s', 'woocommerce-tm-extra-product-options' ), '<a href="' . $this->get_generate_token_url() . '" target="_blank">' . esc_html__( 'clicking this link', 'envato-market' ) . '</a>' ) . '</p>',
