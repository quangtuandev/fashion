...
final class TM_EPO_UPDATE_Licenser {
	...
	public function request( $action = '' ) {
		...
		if ( (boolean) $result->result === FALSE ) {
			...
			if ( !empty( $rs ) ) {
				switch ( $result->code ) {
					...
					case "4":
						...
						break;
					case "5":
						$message = __( 'Trying to activate with an invalid purchase code for your token!', 'woocommerce-tm-extra-product-options' );
						break;
					case "50":
						$message = __( 'Envato rate limit exceeded! Please try again later.', 'woocommerce-tm-extra-product-options' );
						break;
					case "6":
			...
			echo json_encode( array( 'result' => '-2', 'code'=>$result->code, 'message' => sprintf( $message_wrap, $status, $message ) ) );
			die();
		}
