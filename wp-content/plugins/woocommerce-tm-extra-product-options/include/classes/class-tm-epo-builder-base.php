...
final class TM_EPO_BUILDER_base {
	...
	public final function add_setting_price_type( $name = "" ) {
		return array(
			...
			"options"     => array(
				...
				array( "text" => __( 'Price per word', 'woocommerce-tm-extra-product-options' ), "value" => "word" ),
				array( "text" => __( "Percent of the original price per word", 'woocommerce-tm-extra-product-options' ), "value" => "wordpercent" ),
				array( "text" => __( 'Price per word (no n-th char)', 'woocommerce-tm-extra-product-options' ), "value" => "wordnon" ),
				array( "text" => __( "Percent of the original price per word (no n-th char)", 'woocommerce-tm-extra-product-options' ), "value" => "wordpercentnon" ),
	...
	public final function add_setting_date_format( $name = "" ) {
		return array(
			...
			"options" => array(
				...
				array( "text" => __( "Year / Month / Day", 'woocommerce-tm-extra-product-options' ), "value" => "6" ),
				array( "text" => __( "Year / Day / Month", 'woocommerce-tm-extra-product-options' ), "value" => "7" ),
				array( "text" => __( "Year . Month . Day", 'woocommerce-tm-extra-product-options' ), "value" => "8" ),
				array( "text" => __( "Year . Day . Month", 'woocommerce-tm-extra-product-options' ), "value" => "9" ),
				array( "text" => __( "Year - Month - Day", 'woocommerce-tm-extra-product-options' ), "value" => "10" ),
				array( "text" => __( "Year - Day - Month", 'woocommerce-tm-extra-product-options' ), "value" => "11" ),
	...
	public function builder_sub_variations_options( $meta = array(), $product_id = 0 ) {
		...
		if ( empty( $attributes ) ) {
			$out .= '<div class="errortitle"><p><i class="tcfa tcfa-exclamation-triangle"></i> ' . __( 'No saved variations found.', 'woocommerce-tm-extra-product-options' ) . '</p></div>';
			return $out;
		}
	...
	private function builder_sub_options( $options = array(), $name = "multiple_selectbox_options", $counter = NULL, $default_value = NULL ) {
		...
		foreach ( $options["title"] as $ar => $el ) {
			...
			$o["image"]["extra"] = $upload . '<span class="s tm_upload_image"><img class="tm_upload_image_img" alt="" src="' . $options["image"][ $ar ] . '" />' . '<button rel="tc-option-image" class="tc-button small builder-image-delete" type="button"><i class="tcfa tcfa-times"></i></button>' . '</span>';
			...
			$o["imagec"]["extra"] = $uploadc . '<span class="tm_upload_image tm_upload_imagec"><img class="tm_upload_image_img" alt="" src="' . $options["imagec"][ $ar ] . '" />' . '<button rel="tc-option-imagec" class="tc-button small builder-image-delete" type="button"><i class="tcfa tcfa-times"></i></button>' . '</span>';
			...
			$o["imagep"]["extra"] = $uploadp . '<span class="tm_upload_image tm_upload_imagep"><img class="tm_upload_image_img" alt="" src="' . $options["imagep"][ $ar ] . '" />' . '<button rel="tc-option-imagep" class="tc-button small builder-image-delete" type="button"><i class="tcfa tcfa-times"></i></button>' . '</span>';
			...
			$o["imagel"]["extra"] = $uploadl . '<span class="tm_upload_image tm_upload_imagel"><img class="tm_upload_image_img" alt="" src="' . $options["imagel"][ $ar ] . '" />' . '<button rel="tc-option-imagel" class="tc-button small builder-image-delete" type="button"><i class="tcfa tcfa-times"></i></button>' . '</span>';
			...
			if ( $name == "multiple_checkboxes_options" ) {
				$default_select = '<input type="checkbox" value="' . $d_counter . '" name="tm_meta[tmfbuilder][' . $name . '_default_value][' . (is_null( $counter ) ? 0 : $counter) . '][]" class="tm-default-checkbox" ' . checked( (is_null( $counter ) ? "" : isset( $default_value[ $counter ] ) ? is_array($default_value[ $counter ]) &&  in_array( $d_counter, $default_value[ $counter ] ) : ""), TRUE, 0 ) . '>';
			} else {
