...
final class TM_Extra_Product_Options {
	...
	public function tm_woocommerce_get_price( $price = 0, $product = FALSE ) {
		...
		if ( ! $this->wc_vars['is_product'] && $this->tm_epo_use_from_on_price == "no" ){
			if ( $price === '' ) {
				$price = 0;
				if ( !empty( $max ) ) {
	...
	public function tm_get_price_html( $price = '', $product = FALSE ) {
		...
		if ( !TM_EPO_API()->is_valid_options( $has_epo ) || $type == 'variation') {

			//remove_all_filters('woocommerce_get_price_html');
			remove_filter('woocommerce_get_price_html', array( $this, 'tm_get_price_html' ), 1, 2);
		...
		if ( $type == 'variable' || $type == 'variable-subscription' ) {
			...
			// include taxes
			$min = $this->tc_get_display_price( $product, $min );	
			...
			$max = $this->tc_get_display_price( $product, $max );
			...
			if ( $product->is_on_sale() ) {
				...
				/*$displayed_price = (function_exists( 'wc_get_price_to_display' )
							? wc_format_sale_price( wc_get_price_to_display( $product, array( 'price' => $min_regular_price ) ), wc_get_price_to_display( $product, array( 'price' => $min_price ) ) ) 
			...
			if ( $this->tc_get_price( $product ) > 0 ) {
				...
				if ( $product->is_on_sale() && $this->tc_get_regular_price( $product ) ) {
					if ( $use_from && ($max > 0 || $max > $min) ) {
						...
						$displayed_price = (function_exists( 'wc_get_price_to_display' )
							? wc_format_sale_price( wc_get_price_to_display( $product, array( 'price' => $display_regular_price ) ), wc_get_price_to_display( $product, array( 'price' => $display_price ) ) ) 
	...
	public function tm_epo_option_name( $label = "", $args = NULL, $counter = NULL, $value = NULL, $vlabel = NULL ) {
		...
		return apply_filters( 'wc_epo_label' , do_shortcode( $label ) );
			...
			foreach ( $filtered_array as $section ) {
				...
				if ( $this->tm_epo_cart_field_display != "link" ) {
					if ( empty( $section['hidelabelincart'] ) || $section['hidevalueincart'] === 'noprice' || empty( $section['hidevalueincart'] ) ) {
						$value_to_show = (empty( $section['hidevalueincart'] ) || $section['hidevalueincart'] === 'noprice') ? apply_filters( 'wc_epo_label_in_cart' , do_shortcode( TM_EPO_HELPER()->html_entity_decode( $value ) ) ) : '';
						...
						$other_data[] = array(
							...
							'tm_value'       => apply_filters( 'wc_epo_label_in_cart' , do_shortcode( TM_EPO_HELPER()->html_entity_decode( $value ) ) ),
		...
		if ( $this->tm_epo_cart_field_display == "link" ) {
			...
			foreach ( $link_data as $link ) {
				$data .= '<div class="tm-row tm-cart-row">'
					...
					. '<div class="tm-cell col-4 cpf-value">' . apply_filters( 'wc_epo_label_in_cart' , do_shortcode( TM_EPO_HELPER()->html_entity_decode( $link['value'] ) ) ) . '</div>'
		...
		if ( $per_product_pricing ) {
			...
			switch ( $_price_type ) {
				...
				case 'percentcurrenttotal':
					...
					break;
				case 'word':
					$_price = floatval( $_price * TM_EPO_HELPER()->count_words( $post_data[ $attribute ] ) );
					if ( $currency ) {
				...
				case 'wordpercent':
					if ( $cpf_product_price !== FALSE ) {
						...
						$_price = floatval( TM_EPO_HELPER()->count_words( $post_data[ $attribute ] ) ) * (($_price / 100) * floatval( $cpf_product_price ));
					}
					break;
				case 'wordnon':
					$freechars = absint( $element['freechars'] );
					$_textlength = floatval( TM_EPO_HELPER()->count_words( $post_data[ $attribute ] ) ) - $freechars;
					if ( $_textlength < 0 ) {
						$_textlength = 0;
					}
	...
	public function order_again_cart_item_data( $cart_item_meta, $item, $order ) {
		...
		if ( isset($cart_item_meta['tmcartepo']) || isset($cart_item_meta['tmsubscriptionfee']) || isset($cart_item_meta['tmcartfee']) ){
	...
	public function get_product_tm_epos( $post_id = 0, $form_prefix = "" ) {
		...
		if ( !$this->tm_meta_cpf['exclude'] ) {
			...
			//$meta_array2 = TM_EPO_HELPER()->build_meta_query( 'AND', 'tm_meta_product_exclude_ids', '"' . $post_id . '";', 'NOT LIKE', 'EXISTS' );
			$meta_array2 = TM_EPO_HELPER()->build_meta_query( 'OR', 'tm_meta_product_exclude_ids', '"' . $post_id . '";', 'NOT LIKE', 'NOT EXISTS' );
			...
			//$meta_query_args = TM_EPO_HELPER()->build_meta_query( 'OR', 'tm_meta_disable_categories', 1, '!=', 'NOT EXISTS' );
			$args = array(
		...
		$epos = $this->generate_global_epos( $tmglobalprices, $post_id, $this->tm_original_builder_elements, $variations_for_conditional_logic );
	 ...
	 * @return array
	 */
	public function generate_global_epos( $tmglobalprices, $post_id, $tm_original_builder_elements, $variations_for_conditional_logic = array() ) {
		$global_epos = array();
		$epos_prices = array();
		...
		if ( $tmglobalprices ) {
			...
			foreach ( $tmglobalprices as $price ) {
				...
				$tm_meta_product_ids = tc_get_post_meta( $tmcp_id, 'tm_meta_product_ids', TRUE );
			...
			foreach ( $extra_section_logic as $section_logic ) {
				...
				if ( !empty( $global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]["sections_logic"] ) ){
					...
					if (is_object($current_section_logic)){
						$current_section_logic["what"] = "all";
						$current_section_logic["rules"][] = $rule;
						$global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]["sections_clogic"] = json_encode($current_section_logic);
						$global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]["sections_logic"] = "1";
					}												
				}else{
					$global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]["sections_clogic"] = json_encode($section_logic["extra_logic"]);
					$global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]["sections_logic"] = "1";
				}
			}
		}
