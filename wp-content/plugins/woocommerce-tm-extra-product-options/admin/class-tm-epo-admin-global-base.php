...
final class TM_EPO_ADMIN_Global_base {
		...
		if ( $post_type == 'product' ) {
			...
			switch ( $do_action ) {
				...
				case 'tcproductclear':
					$tc_removed = 0;
					foreach ( $post_ids as $post_id ) {
						tc_update_post_meta( $post_id, 'tm_meta', '' );						
						$tc_removed++;
					}
					$sendback = add_query_arg( array( 'tc_removed' => $tc_removed, 'ids' => join( ',', $post_ids ) ), $sendback );
					break;
	...
	public function bulk_actions_edit_product( $actions ) {
		...
		$actions['tcproductclear'] = __( 'Delete all options on the product', 'woocommerce-tm-extra-product-options' );
	...
	public function tm_list_columns( $columns ) {
		...
		$new_columns['product_exclude_ids'] = __( 'Excluded Products', 'woocommerce-tm-extra-product-options' );
	...
	public function tm_add_metaboxes() {
		...
		// Products meta box
		add_meta_box( "tm_product_exclude", __( 'Exclude Products', 'woocommerce-tm-extra-product-options' ), array( $this, 'tm_product_exclude_meta_box' ), NULL, "side", "core" );
	...
	public function tm_product_exclude_meta_box( $post ) {
		...
		?>
        <label for="tm_product_exclude_ids"><?php _e( 'Exclude Product(s) from the form', 'woocommerce-tm-extra-product-options' ); ?></label>
		<?php if ( version_compare( WC()->version, '2.3', '<' ) ) { ?>
        <select <?php echo $disabled; ?>id="tm_product_exclude_ids"
